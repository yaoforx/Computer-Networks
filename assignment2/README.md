...
## Background Reading
Begin by reading pages 240-258 in the textbook *Computer Networks: A Systems Approach* (5th edition) by Peterson & Davie.  An excerpt of these pages is included in the `assignment2` directory with the filename `textbook_reading.pdf`.
...
### Restrictions
There are limitations on what information your `DVrouter` and `LSrouter` classes are allowed to access from the other provided Python files. Unlike C and Java, Python does not support private variables and classes. This means you have the following limitations.
